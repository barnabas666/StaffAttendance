...
public class StaffPostgresDataProcessor : IStaffDataProcessor
{
    ...
    public StaffPostgresDataProcessor(IPostgresDataAccess db, IConnectionStringData connectionStringData)
    {
        _db = db;
    ...
    {
        string sql = await QueryHelper.LoadPostgresQueryAsync("StaffPhoneNumbers_Check.sql");
        List<bool> output = await _db.LoadDataAsync<bool, dynamic>(
            sql,
            new { staffId, phoneNumberId },
            _connectionStringName);
