...
- **Web App**: Staff registration, profile management, and administration via a user-friendly web interface. All authentication and user management logic is handled by the API; the web app provides only the UI pages for these actions.
...
- **Secure Authentication**: Uses ASP.NET Core Identity (implemented in the API) and JWT tokens for secure access.
...
- **StaffAtt.Web**: ASP.NET Core Razor Pages web application (UI for staff and admins; authentication UI only).
- **StaffAtt.Desktop**: WPF desktop application (portable, connects via API).
- **StaffAttApi**: ASP.NET Core Web API (all business logic, data access, and authentication).
- **StaffAttLibrary**: Core business logic and data access (used only by API).
- **StaffAtt.Identity**: Identity system for authentication (used exclusively by API).
...
- **Main Staff Database**:
  - PostgreSQL (default, recommended for deployment; queries in `/StaffAttLibrary/Queries/PostgreSQL/`)
  - SQL Server (with stored procedures, scripts in `/StaffAttDB/`)
  - SQLite (queries in `/StaffAttLibrary/Queries/SQLite/`)
- **Identity Database**:
  - SQLite (for authentication, configured in `StaffAttApi/appsettings.json`)
